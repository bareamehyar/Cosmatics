...
@section("content")
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="tile">
                <div class="tile-title-w-btn"><a href="{{route('items.add_ons.create', [$item->id,app()->getLocale()])}}" class="btn btn-primary">Create New Add On's Category</a></div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered" id="sampleTable">
                            ...
                            <tr>
                                ...
                                <th>Arabic name</th>
                                <th>Choice Type</th>
                                @if(hasPermissions(["edit-add-on's", "delete-add-on's"]))
                                <th>Control</th>
                                @endif
                            </tr>
                            </thead>
                            ...
                            @foreach($addOns as $addOn)
                                <tr>
                                    ...
                                    <td>{{$addOn->which_choice == 1 ? "Single" : "Multi"}}</td>
                                    @if(hasPermissions(["edit-add-on's", "delete-add-on's"]))
                                    <td>
                                        @if(hasPermissions("edit-add-on's"))
                                        <a href="{{route("items.add_ons.edit", [$item->id, $addOn->id,app()->getLocale()])}}" class="control-link edit"><i class="fas fa-edit"></i></a>
                                        @endif
                                        @if(hasPermissions("delete-add-on's"))
                                        <form action="{{route("items.add_ons.destroy", [$item->id, $addOn->id,app()->getLocale()])}}" method="post" id="delete{{$addOn->id}}" style="display: none" data-swal-title="Delete Add On" data-swal-text="Are Your Sure To Delete This Add On ?" data-yes="Yes" data-no="No" data-success-msg="the Add On has been deleted succssfully">@csrf @method("delete")</form>
