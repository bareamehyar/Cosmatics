    ...
    <div class="row">
        <div class="col-lg-10 m-auto">
            <div class="tile">
                ...
                <div class="tile-body">
                    <form method="post" action="{{route("sliders.store",app()->getLocale())}}" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-lg-3 col-md-5 col-sm-6">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control action-select @if($errors->has('category')) is-invalid @endif" id="NavigateList" name="category" data-type="category">
                                            ...
                                            @foreach($categories as $category)
                                                <option value="{{$category->id}}" @if(old("category")) {{selected("navigate_id", $category->id)}} @endif >{{$category->category_name_en}} ( {{$category->category_name_ar}} )</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @error("category")
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control action-select @if($errors->has('item')) is-invalid @endif" id="NavigateList" name="item" data-type="item">
                                            ...
                                            @foreach($items as $item)
                                                <option value="{{$item->id}}" @if(old("item")) {{selected("navigate_id", $item->id)}} @endif>{{$item->item_name_en}} ( {{$item->item_name_ar}} )</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @error("item")
