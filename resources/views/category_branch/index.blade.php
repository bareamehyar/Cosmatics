...
@section("page-nav-title")
    <div class="app-title">
        <div>
            ...
            <p>Control and view all Categories Branches</p>
        </div>
        <ul class="app-breadcrumb breadcrumb">
            <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
            <li class="breadcrumb-item"><a href="#">Categories Branhces</a></li>
        </ul>
    </div>
@endsection
@section("content")
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered" id="sampleTable">
                            ...
                            @foreach($categories as $category)
                                <tr>
                                    <td>{{$category->id}}</td>
                                    <td><img src="{{$category->img_url}}" alt="" style="width: 150px"></td>
                                    <td>{{$category->name_en}}</td>
                                    <td>{{$category->name_ar}}</td>
                                    <td>{{$category->status == 1 ? "Active" : "Non-Active"}}</td>
                                    @if(hasPermissions(["edit-category-branch", "delete-category-branch"]))
                                    <td>
                                        @if(hasPermissions("edit-category-branch"))
                                        <a href="{{route("categories_branches.edit",[$category->id , app()->getLocale()])}}" class="control-link edit"><i class="fas fa-edit"></i></a>
                                        @endif
                                        @if(hasPermissions("delete-category-branch"))
                                        <form action="{{route("categories_branches.destroy",[$category->id,app()->getLocale()])}}" method="post" id="delete{{$category->id}}" style="display: none" data-swal-title="Delete Category" data-swal-text="Are Your Sure To Delete This Category ?" data-yes="Yes" data-no="No" data-success-msg="the category has been deleted succssfully">@csrf @method("delete")</form>
